...
export default function ReadWatchPage() {
    ...
    const [filter, setFilter] = useState<'all' | 'watching' | 'queued' | 'finished'>('all');
    const [loading, setLoading] = useState(true);

    const fetchItems = async () => {
        const data = await getMediaItems();
        setItems(data.filter(i => i.type === 'movie' || i.type === 'tv'));
        setLoading(false);
    };
        ...
        <div className="min-h-screen pt-24 pb-12 px-6 lg:px-12 space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                <div className="space-y-1">
                    <h1 className="text-3xl font-bold text-white tracking-tight">To Watch</h1>
                    <p className="text-white/60">Movies and TV shows.</p>
                </div>

                <div className="flex items-center gap-4">
                    <div className="flex p-1 bg-white/5 rounded-full border border-white/10">
                        {['all', 'watching', 'queued', 'finished'].map((f) => (
                            <button
                                key={f}
